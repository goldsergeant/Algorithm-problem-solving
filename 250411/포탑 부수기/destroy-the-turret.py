...
def attack_by_lazer(attack, path):
...
def attack_by_bomb(w_r, w_c, s_r, s_c, attack):
...
for cur_turn in range(1, K + 1):
    ...
    recent_attacks[w_r][w_c] = cur_turn
    ...
    if not lazer_path:  # 포탑 공격
        ...
        attack_by_bomb(w_r, w_c, s_r, s_c, canons[w_r][w_c])
    else:
        attack_by_lazer(canons[w_r][w_c], lazer_path)
